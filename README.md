...
```bash
# 1. 创建数据库
mysql -u root -p -e "CREATE DATABASE mitedtsm_database DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;"

# 2. 执行基础数据库脚本
mysql -u root -p mitedtsm_database < database/base/ruoyi-vue-pro.sql

# 3. 执行新增模块脚本（按顺序）
mysql -u root -p mitedtsm_database < database/new/*.sql

# 4. 执行Server目录下的SQL脚本
mysql -u root -p mitedtsm_database < Server/sql/mysql/*.sql
```
